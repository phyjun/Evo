...
def load_checkpoint(
    ...
    *args, **kwargs
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        config=model_config,
        trust_remote_code=True,
        torch_dtype=torch.float32,  # Use FP32 since we're not using Flash Attention
        revision='1.1_fix' if re.match(r'evo-1-.*-base', model_name) else 'main',
    )
    ...
    # Disable all Flash operations in config
    global_config.use_flash_attn = False
    global_config.use_flash_rmsnorm = False
    global_config.use_flash_depthwise = False
    global_config.use_flashfft = False

    # Load SH Model.

    model = StripedHyena(global_config)
    model.load_state_dict(state_dict, strict=True)
    # Skip BF16 conversion
